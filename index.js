...
const
    ...
    app=express().use(bodyParser.json()); //creates express http sever

//Sets server port and logs message on success
app.listen(process.env.PORT || 1337, ()=> console.log('webhook is listening'));


//Creates the endpoint for our webhoook
app.post('/webhook', (req,res) => {
    let body=req.body;

    //Checks this an event from a page subscription
    if(body.object === 'page'){
        //Iterate over each entry -there may be multiple if batched
        body.entry.forEach(function(entry) {

            //Gets the message entry messaging is an array ,but will only over contain one message so we get index 0
        ...
        //Return a 404 ot found if event is not from a page subscription
        res.sendStatus(404);
    }
